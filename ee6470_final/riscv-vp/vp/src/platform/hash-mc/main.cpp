...
public:
	...
	addr_t HASH0_start_addr = 0x73000000;
 	addr_t HASH0_size = 0x01000000;
 	addr_t HASH0_end_addr = HASH0_start_addr + HASH0_size - 1;
	addr_t HASH1_start_addr = 0x74000000;
 	addr_t HASH1_size = 0x01000000;
 	addr_t HASH1_end_addr = HASH1_start_addr + HASH1_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > HASH1_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	DebugMemoryInterface core0_dbg_if("DebugMemoryInterface0");
	DebugMemoryInterface core1_dbg_if("DebugMemoryInterface1");
	HASH HASH0("HASH0");
	HASH HASH1("HASH1");
	...
	core0.init(&core0_mem_if, &core0_mem_if, &clint, entry_point, rv32_align_address(opt.mem_end_addr));
	core1.init(&core1_mem_if, &core1_mem_if, &clint, entry_point, rv32_align_address(opt.mem_end_addr));
	...
	bus.ports[12] = new PortMapping(opt.HASH0_start_addr, opt.HASH0_end_addr);
	bus.ports[13] = new PortMapping(opt.HASH1_start_addr, opt.HASH1_end_addr);
	...
	bus.isocks[12].bind(HASH0.tsock);
	bus.isocks[13].bind(HASH1.tsock);
...
public:
	...
	addr_t sys_start_addr = 0x02010000;
	addr_t sys_end_addr = 0x020103ff;
	addr_t HASH0_start_addr = 0x30000000;
	addr_t HASH0_size = 0x10000000;
	addr_t HASH0_end_addr = HASH0_start_addr + HASH0_size - 1;
	addr_t HASH1_start_addr = 0x40000000;
	addr_t HASH1_size = 0x10000000;
	addr_t HASH1_end_addr = HASH1_start_addr + HASH1_size - 1;
...
int sc_main(int argc, char **argv) {
	...
	bus.ports[4] = new PortMapping(opt.HASH0_start_addr, opt.HASH0_end_addr);
	bus.ports[5] = new PortMapping(opt.HASH1_start_addr, opt.HASH1_end_addr);
	...
	bus.isocks[4].bind(HASH0.tsock);
	bus.isocks[5].bind(HASH1.tsock);
